...
/**
 * Query Wikipedia REST API for a golfer's page, photo, and bio.
 */
async function tryWikipedia(name) {
  ...
  for (const title of candidates) {
    try {
      ...
      // MediaWiki action API with pageimages + extracts
      ...
      const res = await fetch(url, {
        headers: { 'User-Agent': 'FantasyGolfEnrichment/1.0 (private project; contact@example.com)' },
      });
      if (!res.ok) continue;
      ...
      // Validate it's actually a golfer
      ...
      // Also try REST API for a better summary if action API gave us something
      let photoUrl = page.original?.source || page.thumbnail?.source || null;
      let bio = extract || null;

      // Try the summary REST API for better bio
      ...
      if (summaryRes.ok) {
        ...
        if (summaryData.type === 'standard' && !summaryData.title.includes('Not found')) {
          // Prefer REST API extract and thumbnail
          bio = summaryData.extract || bio;
          photoUrl = summaryData.thumbnail?.source || summaryData.originalimage?.source || photoUrl;
        }
      }
...
async function main() {
  ...
  // If fewer than BATCH_SIZE without bio, fill with any null-photo players
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    try {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        STATS.skipped++;
        console.log(`  → found but no new data to update`);
      }
    } catch (e) {
      STATS.errors++;
